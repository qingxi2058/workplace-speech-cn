...
redskill 表单有三个字段，内容和用途完全不同：

| 字段 | 给谁看 | 写法 |
|------|--------|------|
| 简介 | 用户扫一眼决定要不要用 | 2-3 句散文，说清楚解决什么问题，有温度 |
| Skill 介绍 | 用户细看了解产品 | 产品页逻辑：是什么 → 怎么工作 → 覆盖场景 → 不做什么 |
| SKILL.md（Skill介绍字段粘贴内容） | Claude 读，用来执行 | 指令逻辑，是 Claude 的行为规范，不是给用户看的 |

> **当前 skill 的表单内容已写好，直接复制：** `redskill/FORM-CONTENT.md`
...
工作方式
[说清楚 skill 的响应流程，2-3句]

覆盖场景
[主要场景列表，逗号分隔]

不做什么
[边界声明，说清楚 skill 不干什么，建立信任]
```
